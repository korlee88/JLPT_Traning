...
- **Two batch-3 entries were read off a smudged print and inferred, not clearly seen:** `貸す`/かす/빌려주다 and `風`/かぜ/바람. Each was pinned by three signals — the printed Korean meaning, the alphabetical slot (かじ → かす → かぜ), and the partial glyph — but if the owner ever says a kanji-quiz entry looks wrong, check these two first.

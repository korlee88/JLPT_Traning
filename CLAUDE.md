...
- **Where the list stands:** p.17 (あ행) is complete — items 1–20 (`青い` … `以上`) went in 2026-09-14 as batch 1 (#10), items 21–39 (`急ぐ` … `送る`) on 2026-09-15 as batch 2 (#13). Batch 3 on 2026-09-16 took p.18's left column: the あ행 tail (`起こす` … `音楽`) plus か행 through `考える`. Batch 4 on 2026-09-21 took the middle column, `北` … `声`. Batch 5 on 2026-09-22 took p.19's left column, `氷` … `女性` — the か행 tail (`氷`/`心`/`答える`/`今度`) plus さ행 through `女性`, skipping `出発` (already in the file from the early hand-authored entries). The next batch picks up at `知る`, the last row of that column; the right column then opens with `白い`/`人口`/`親切だ`/`新聞社`, and た행 begins at `大使館`.

...
- **Where the list stands:** p.17 (あ행) is complete — items 1–20 (`青い` … `以上`) went in 2026-09-14 as batch 1 (#10), items 21–39 (`急ぐ` … `送る`) on 2026-09-15 as batch 2 (#13). Batch 3 on 2026-09-16 took p.18's left column: the あ행 tail (`起こす` … `音楽`) plus か행 through `考える`. Batch 4 on 2026-09-21 took the middle column, `北` … `声`. The next batch picks up after `声` — the following column opens with `氷`/`心`/`答える`/`今度`, then the さ행 header.
...
- **The textbook's 「합격 어휘 확인 문제」 drills are exercises, not a word list.** The owner sent a scan of them on 2026-09-21 (a/b two-choice: 한자→읽기, 뜻→동사, 뜻→부사, 뜻→가타카나). The words in them are fair game — that's dictionary fact, same as the daily batches — but the *items* are not: which word is tested and, especially, which wrong option was chosen as its trap is the book's editorial work. `loanword.json` came out of that scan and holds only the word list; the app generates its own distractors. Do the same for any future drill scan.
